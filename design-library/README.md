...
| `TreatedImage` | Photo → duotone/halftone in the project palette | seed |
| `HexCell` | Honeycomb cell: a hexagon holding a solid brand color or a duotone image + optional centered content — for "network / cellular / interlocking" motifs | dirittoeconsulenza.it |

Treatments (`treatments/*.css`): `.dl-sketch-frame`, `.dl-treated-duotone`,
`.dl-treated-halftone`, `.dl-grain`, `.dl-hex` / `.dl-hex-flat` / `.dl-hexcell`.
Tokens: `tokens/tokens.css` (`--dl-*`).
