...
Caps: 20 files / 2MB each; tiny files and non-images are auto-dropped. Then:
- Grab the **logo** yourself (header `<img>`/`<svg>`, or filename/alt/class containing "logo" — often an SVG the script skips) via curl into `assets/images/logo.<ext>`.
- If the script yielded few images but pages/*.md list important ones (hero, team, office), curl the top ones manually.
